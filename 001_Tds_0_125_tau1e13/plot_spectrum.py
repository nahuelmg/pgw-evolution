...
if h1_initial:
    ...
    np.savetxt('spectrum_100_Tds_0_125_tau1e13',spectrum)
...
if h3_initial:
    ...
    if reg_r0:
        ...
        np.savetxt('spectrum_001_Tds_0_125_tau1e13',spectrum)
...
np.savetxt('kc_vec_Tds_0_125_tau1e13',kc_vec)

np.savetxt('Neff_today_Tds_0_125_tau1e13',[Neff_today, Neff_today])
...
plt.loglog(kc_vec/(2.*np.pi),spectrum,label=r'${\rm Producción}$ ${\rm de}$ ${\rm GW}$')
plt.loglog(kc_vec[tuple([kc_vec/(2*np.pi)>1e-16])]/(2.*np.pi),10**(-47.4)*(kc_vec[tuple([kc_vec/(2*np.pi)>1e-16])]/(2.*np.pi))**3,label=r'$\propto f^3$',linewidth=1.5,linestyle='--',color='black')
...
plt.tight_layout()
plt.savefig('GW_spectrum_001_Tds_0_125_tau1e13')
