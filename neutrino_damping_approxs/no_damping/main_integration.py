...
from params import h1_initial, h2_initial, h3_initial, tau, step_max_num, u_max_num, do_s_interaction, do_neutrino_damping, do_viscosity, spectrum_not_today, do_neutrino_damping_2, do_neutrino_damping_3
...
for i in range(i_initial, i_final):
    ...
    if do_s_interaction and do_neutrino_damping:
    ...
    elif do_s_interaction and h1_initial:
    ...
    elif do_s_interaction and h3_initial:
    ...
    elif do_neutrino_damping_2:
        
        f_nu_u = uspl(u,f_nu,k=4,s=0)
        
        viscosity_nu = uspl(u,24*f_nu_u(u)*(dadu_a_u(u))**2.*((np.heaviside((u-u_nu)-1e-2,1))*(1/15)),k=4,s=0)
        # en viscosity_nu no se puede usar el smoothstep porque es algo que pega bien así
        ...
        def func(u,y):
            h1, h2 = y
            dydu = [ h2, - h1 - 2. * dadu_a_u(u) * h2 - step_function_for_nu(u) * viscosity_nu(u) * ( h1 - 1 ) ]
            return dydu
        ...
        if kc < 1 / ( 100 * tau ) :
            ...
            viscosity_nu = uspl(u,f_nu_u(u)*(dadu_a_u(u))**2.*((np.heaviside((u-u_nu)-1e-2,1))*(1/15)),k=4,s=0)
            # en viscosity_nu no se puede usar el smoothstep porque es algo que pega bien así
            ...
            def func(u,y):
                h1, h2, h3 = y
                dydu = [ h2, - h1 - 2. * dadu_a_u(u) * h2 + f_nu_u(u) * (dadu_a_u(u))**2. * step_function_for_nu(u) * h3, - 1. / ( kc * tau ) * h3 - 24/15 * h2 ]
                return dydu
