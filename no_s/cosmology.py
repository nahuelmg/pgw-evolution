    ...
    if do_cosmo_s:
        ...
        if do_s_massive: # REVISAAAAR BIEN 
            ...
            if kill_interaction_after_decoupling:
                ...
                f_s = deg_s/2 * omega_gamma0 / ( g * omega_gamma0 / 2 + omega_m0 * a * ( gs / gs[-1] )**(4/3) + omega_lambda0 * a**4 * ( gs / gs[-1] )**(4/3) ) * ( step(T-Tds,0.05*Tds,0) + step(-T+Tds,0.05*Tds,1) * ( gs_SM / gs_SM_T(Tds) )**(4/3))
...
#plt.figure()
#plt.semilogx(frequency_from_horizon_crossing,g_a(a))
